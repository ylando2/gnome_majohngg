...
public class Game
{
    ...
    public Game (Map map)
    {
        ...
        uint n_sets=tiles.length () / 4;
        sets = new List<Tile>[n_sets];
        for (var i=0; i < n_sets; i++) {
            sets[i]=new List<Tile>();
        }
        foreach (var tile in tiles) {
            sets[tile.set].append(tile);
        }
    }
    ...
    //Run only if there is an option for auto_move after the game start and after every move.
    public void auto_move ()
    {
        var changed=false;
        var n_sets=tiles.length()/4;
        do {
            changed=false;
            for (var i=0; i<n_sets; i++) {
                var can_remove_set=true;
                foreach (var tile in sets[i]) {
                    if (tile.visible==true && !tile_can_move(tile)) {
